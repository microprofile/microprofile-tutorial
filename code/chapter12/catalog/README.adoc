...
NOTE: The `id` argument maps to a Java `Long`, which SmallRye GraphQL (the MicroProfile GraphQL runtime) exposes as the `BigInteger` scalar — not `Int` or `ID`. Using any other type causes a validation error.
