...
The Inventory Service is a microservice built with Jakarta EE 10.0 and MicroProfile 7.1, showcasing comprehensive REST client integration patterns with the Catalog Service. This service demonstrates three different approaches to MicroProfile Rest Client usage:
...
curl -X GET "http://localhost:7050/inventory/api/inventories?maxQuantity=100"

# Paginate results
curl -X GET "http://localhost:7050/inventory/api/inventories?page=0&size=10"
----

== Automated Tests

JUnit tests are provided under `src/test/java`:

* **`InventoryServiceTest`** - Unit tests for `InventoryService` business logic
* **`InventoryServiceIntegrationTest`** - Integration-style tests exercising the service layer
...
**RestClientBuilder Configuration** (programmatic, using the MP Rest Client 4.0 `baseUri(String)` convenience method):
[source,java]
----
...
* **Swagger UI**: http://localhost:7050/openapi/ui
* **OpenAPI JSON**: http://localhost:7050/openapi
...
│   │   │       ├── client/                    # MicroProfile Rest Client interface
...
│   │   ├── resources/
│   │   │   └── META-INF/
│   │   │       └── microprofile-config.properties  # REST client configuration
