...
* Annotate the method with `@Asynchronous` and `@Timeout` using the default or a baseline value:

NOTE: Always combine `@Timeout` with `@Asynchronous`. Without `@Asynchronous`, timeout enforcement relies on thread interruption, which is unreliable for blocking operations such as JDBC calls or network I/O. The method return type must be `CompletionStage<T>` or `Future<T>`, and the fallback method must match that signature.
...
public class ProductService {
    ...
    public CompletionStage<List<Product>> getProducts() {
        return CompletableFuture.completedFuture(productRepository.findAllProducts());
    }

    public CompletionStage<List<Product>> getProductsFromCache() {
        return CompletableFuture.completedFuture(
            productCache.getAll().stream()
                .map(obj -> (Product) obj)
                .collect(Collectors.toList()));
    }
}
